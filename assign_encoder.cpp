...
void Encode(int n, char* s[])
{
    ...
    char input_file_data[10*1024];
    char output_file_data[2*10*1024];

    FILE* input_file=fopen(input_file_name, "rb");
    if(input_file==0) {printf("Cannot open the input file (%s).\n", input_file_name); fflush(stdout); return;}

    int ind=0, cnt=fread(input_file_data, 1, 10*1024, input_file);
    if(cnt<=0) {printf("Cannot read the input file (%s).\n", input_file_name); fflush(stdout); return;}
    if(cnt>=10*1010) {printf("The input file size must be less than 10,000 characters.\n"); fflush(stdout); return;}
